import { getRepoData, saveRepoData, getCached, setCached } from '../utils/index.js';
...
export async function checkRepoUpdate(repoInfo, env) {
  ...
  try {
    let cachedResponse = await getCached(cacheKey);
    if (!cachedResponse) {
      // 缓存未命中，调用 GitHub API
      ...
      // 缓存响应结果
      await setCached(cacheKey, response, cacheTtl);
